...
const MIGRATION: &str =
    include_str!("../../../../database/migrations/0015_pod_provider_voice_adapter.sql");
...
pub fn initialize(connection: &Connection) -> Result<()> {
    ...
    let count: i64 = connection.query_row(
        ...
        count == 1,
        "POD provider migration is not registered exactly once"
    );
...
async fn disconnect_handler(
    ...
    tokio::task::spawn_blocking(move || {
        disconnect(&*state.connection()?, request.connection_id.trim())
    })
    .await
    .map_err(task_error)?
...
async fn connect(state: Arc<AppState>, request: PairRequest) -> Result<ConnectionSummary> {
    ...
    let envelope: ProviderEnvelope<PairingData> =
        unsigned_request(&base_url, PAIR_PATH, &payload).await?;
    ensure!(
        envelope.ok,
        "POD pairing was rejected: {}",
        envelope.message
    );
    let paired = envelope.data;
    ensure!(
        paired.provider_id == PROVIDER_KEY,
        "POD pairing returned an unexpected provider"
    );
    ensure!(
        paired.device_token.len() == 64,
        "POD pairing returned an invalid bearer credential"
    );
    ensure!(
        !paired.provider_connection_id.is_empty() && !paired.device_id.is_empty(),
        "POD pairing response is incomplete"
    );
    ensure!(
        paired
        ...
        "POD pairing returned an unsupported capability"
    );
    ...
    if request.make_default {
        tx.execute("UPDATE cloud_connections SET is_default=0", [])?;
    }
    tx.execute(
...
fn save_runtime(connection: &Connection, request: RuntimeUpdateRequest) -> Result<RuntimeProfile> {
    ...
    let provider: Option<String> = connection
        ...
        provider.as_deref() == Some(PROVIDER_KEY),
        "POD connection was not found"
    );
    validate_arguments(&request.transcription_arguments)?;
    validate_arguments(&request.synthesis_arguments)?;
    let transcription_executable = validate_executable(
        request.transcription_enabled,
        request.transcription_executable,
        "transcription",
    )?;
    let synthesis_executable = validate_executable(
        request.synthesis_enabled,
        request.synthesis_executable,
        "synthesis",
    )?;
    ensure!(
        (5..=1800).contains(&request.execution_timeout_seconds),
        "runtime timeout is outside the supported range"
    );
    ensure!(
        (262_144..=16_777_216).contains(&request.maximum_input_bytes),
        "maximum input bytes are outside the supported range"
    );
    ensure!(
        (262_144..=16_777_216).contains(&request.maximum_output_bytes),
        "maximum output bytes are outside the supported range"
    );
    let state = runtime_state(
        request.transcription_enabled,
        transcription_executable.as_deref(),
        request.synthesis_enabled,
        synthesis_executable.as_deref(),
    );
...
async fn run_connection(state: Arc<AppState>, connection_id: &str) -> Result<u64> {
    let record = {
        ...
        record.state != "disconnected" && record.lifecycle_state != "revoked",
        "POD connection is inactive"
    );
    ...
    let heartbeat_due = record
        ...
        .as_deref()
        .and_then(parse_time)
        .map(|value| Utc::now().timestamp() - value >= HEARTBEAT_INTERVAL_SECONDS)
        .unwrap_or(true);
    if heartbeat_due {
        let runtime = {
            let connection = state.connection()?;
        ...
        let envelope: ProviderEnvelope<HeartbeatData> = client
            .signed(Method::POST, HEARTBEAT_PATH, &payload)
            .await?;
        ensure!(
            envelope.ok,
            "POD heartbeat was rejected: {}",
            envelope.message
        );
        let connection = state.connection()?;
    ...
    let envelope: ProviderEnvelope<PollData> =
        client.signed(Method::POST, POLL_PATH, &json!({})).await?;
    ensure!(
        envelope.ok,
        "POD job poll was rejected: {}",
        envelope.message
    );
    {
        let connection = state.connection()?;
    ...
    match result {
        ...
        Err(error) => {
            ...
            let envelope: ProviderEnvelope<Value> =
                ...
                envelope.ok,
                "POD failure receipt was rejected: {}",
                envelope.message
            );
            let remote_state = envelope
                .data
                .get("status")
                .and_then(Value::as_str)
                .unwrap_or(if retryable { "queued" } else { "failed" });
            let local_state = if remote_state == "queued" {
                "retrying"
            } else {
                "failed"
            };
...
async fn execute_job(
    ...
    local_job_id: &str,
    job: &RemoteJob,
) -> Result<(Value, Option<String>, Option<u64>)> {
    let runtime = {
        let connection = state.connection()?;
        runtime_profile(&connection, &client.record.connection_id)?
    };
    {
        let connection = state.connection()?;
    ...
    match job.job_type.as_str() {
        ...
        "speech_to_text" => {
                ...
                "POD transcription runtime is disabled"
            );
            let executable = runtime
                .transcription_executable
                .clone()
                .ok_or_else(|| anyhow!("POD transcription runtime is not configured"))?;
            let artifact = job
                .input_artifact
                .as_ref()
                .ok_or_else(|| anyhow!("POD transcription job has no input artifact"))?;
            ensure!(
            ...
            let envelope: ProviderEnvelope<ArtifactData> =
                ...
                envelope.ok,
                "POD artifact read was rejected: {}",
                envelope.message
            );
            let data = envelope.data;
            ensure!(
                data.artifact_uuid == artifact.artifact_uuid,
                "POD artifact identity mismatch"
            );
            ensure!(
                data.mime_type == artifact.mime_type && data.content_hash == artifact.content_hash,
                "POD artifact metadata mismatch"
            );
            let bytes = STANDARD
                .decode(&data.content_base64)
                .context("POD artifact base64 is invalid")?;
            ensure!(
                bytes.len() == data.plaintext_bytes && bytes.len() <= runtime.maximum_input_bytes,
                "POD artifact size mismatch"
            );
            ensure!(
                sha256(&bytes) == data.content_hash,
                "POD artifact hash mismatch"
            );
            let work = work_directory(state, local_job_id).await?;
            let input = work.join(format!(
                "input.{}",
                extension(&data.mime_type).unwrap_or("audio")
            ));
            ...
            let transcript = result.transcript.unwrap_or_default().trim().to_owned();
                ...
                !transcript.is_empty() && transcript.len() <= 12_000,
                "local transcription result is empty or too large"
            );
        ...
        "text_to_speech" => {
                ...
                runtime.synthesis_enabled,
                "POD synthesis runtime is disabled"
            );
            let executable = runtime
                .synthesis_executable
                .clone()
                .ok_or_else(|| anyhow!("POD synthesis runtime is not configured"))?;
            let text = job
                .payload
                .get("text")
                .and_then(Value::as_str)
                ...
                !text.is_empty() && text.len() <= 6_000,
                "POD synthesis text is empty or too large"
            );
            let format = job
                .payload
                .get("audio_format")
                ...
                ["mp3", "wav", "ogg", "webm"].contains(&format),
                "POD synthesis format is unsupported"
            );
            let work = work_directory(state, local_job_id).await?;
            ...
            let audio_path = result
                ...
                audio_path.starts_with(&work),
                "local synthesis returned an unsafe output path"
            );
            let bytes = fs::read(&audio_path).await?;
            ensure!(
                !bytes.is_empty() && bytes.len() <= runtime.maximum_output_bytes,
                "local synthesis output is empty or too large"
            );
            let mime = result
                .mime_type
...
async fn run_command(
    ...
    let path = Path::new(executable);
        ...
        "local voice runtime executable is unavailable"
    );
    let output = timeout(
    ...
    .await
    .map_err(|_| anyhow!("local voice runtime timed out"))??;
    let stderr = bounded(&String::from_utf8_lossy(&output.stderr), 500);
    ensure!(
        output.status.success(),
        "local voice runtime failed: {stderr}"
    );
    let metadata = fs::metadata(result_path)
        .await
        .context("local voice runtime did not create result JSON")?;
    ensure!(
        metadata.len() as usize <= maximum_output_bytes.min(1024 * 1024),
        "local voice runtime result JSON is too large"
    );
...
fn status_snapshot(connection: &Connection) -> Result<StatusSnapshot> {
    let enabled = connection.query_row(
        "SELECT enabled FROM pod_provider_worker_state WHERE singleton_id=1",
        [],
        |row| row.get::<_, i64>(0),
    )? != 0;
...
fn connection_record(connection: &Connection, id: &str) -> Result<ConnectionRecord> {
    ...
    Ok(ConnectionRecord {
        ...
        display_name: row.1,
        base_url: row.2,
        provider_connection_id: row.3,
        provider_identity_id: row.4,
        provider_display_name: row.5,
        device_id: row.6,
        credential_key: row.7,
        state: row.8,
        lifecycle_state: row.9,
        capabilities: serde_json::from_str(&row.10)?,
        last_heartbeat_at_utc: row.11,
    })
}
    ...
    Ok(statement
        .query_map(params![limit.clamp(1, 250) as i64], |row| {
            Ok(JobSummary {
                ...
                remote_job_uuid: row.get(2)?,
                job_type: row.get(3)?,
                state: row.get(4)?,
                attempt_count: row.get::<_, i64>(5)?.max(0) as u32,
                maximum_attempts: row.get::<_, i64>(6)?.max(1) as u32,
...
impl ProviderClient {
    fn new(record: ConnectionRecord, secrets: StoredSecrets) -> Result<Self> {
            ...
            secrets.device_id == record.device_id
                && secrets.provider_connection_id == record.provider_connection_id,
            "POD credential identity mismatch"
        );
    ...
    async fn signed<T: Serialize + ?Sized, R: DeserializeOwned>(
        ...
        let response = self
            .client
            .request(method, endpoint(&self.record.base_url, path)?)
            .bearer_auth(&self.bearer_token)
            .header("Content-Type", "application/json")
            .header("Accept", "application/json")
            .header("X-POD-Homeserver-ID", &self.record.device_id)
            .header("X-POD-Connection-ID", &self.record.provider_connection_id)
            .header("X-POD-Timestamp", timestamp)
            .header("X-POD-Nonce", nonce)
            .header("X-POD-Signature", signature)
            .header("X-POD-Homeserver-Version", env!("CARGO_PKG_VERSION"))
            .body(bytes)
            .send()
            .await?;
...
async fn unsigned_request<T: Serialize + ?Sized, R: DeserializeOwned>(
    base: &Url,
    path: &str,
    body: &T,
) -> Result<R> {
    let response = reqwest::Client::builder()
        ...
        .build()?
        .post(endpoint(base.as_str(), path)?)
        .header("Content-Type", "application/json")
        .header("Accept", "application/json")
        .json(body)
        .send()
...
async fn parse_response<R: DeserializeOwned>(response: reqwest::Response) -> Result<R> {
    ...
    if !status.is_success() {
        let message = serde_json::from_slice::<Value>(&bytes)
            ...
            .unwrap_or_else(|| format!("POD provider returned HTTP {status}"));
...
fn normalize_url(value: &str) -> Result<Url> {
    let mut url = Url::parse(value.trim()).context("enter a valid POD URL")?;
        ...
        "POD URL must not contain credentials"
    );
    ensure!(
        url.query().is_none() && url.fragment().is_none(),
        "POD URL must not contain query or fragment"
    );
    let host = url.host_str().unwrap_or_default().to_ascii_lowercase();
    let local = matches!(host.as_str(), "localhost" | "127.0.0.1" | "::1");
    ensure!(
        url.scheme() == "https" || (url.scheme() == "http" && local),
        "POD URL must use HTTPS outside localhost tests"
    );
    url.set_path("");
    url.set_query(None);
...
fn endpoint(base: &str, path: &str) -> Result<Url> {
    let mut url = normalize_url(base)?;
    url.set_path(path);
    Ok(url)
}
...
fn error_code(message: &str) -> String {
    ...
    if value.contains("not configured") || value.contains("disabled") {
        "pod_runtime_unconfigured".to_owned()
    } else if value.contains("timed out") {
        "pod_runtime_timeout".to_owned()
    } else if value.contains("artifact") {
        "pod_artifact_invalid".to_owned()
    } else if value.contains("too large") {
        "pod_runtime_output_too_large".to_owned()
    } else {
...
fn validate_job(record: &ConnectionRecord, job: &RemoteJob) -> Result<()> {
        ...
        job.lease_token.len() == 64 && job.lease_token.chars().all(|c| c.is_ascii_hexdigit()),
        "POD lease token is invalid"
    );
    ensure!(
        sha256(&serde_json::to_vec(&job.payload)?) == job.payload_hash,
        "POD payload hash mismatch"
    );
    let capability = match job.job_type.as_str() {
        "speech_to_text" => "pod.voice.transcription.v1",
        "text_to_speech" => "pod.voice.synthesis.v1",
        _ => "pod.voice.jobs.v1",
    };
    ensure!(
        record.capabilities.iter().any(|value| value == capability),
        "POD capability is not granted"
    );
    Ok(())
}
...
fn validate_executable(
    ...
    if enabled {
        let path = value
            .as_deref()
            .map(Path::new)
            .ok_or_else(|| anyhow!("{label} executable is required"))?;
        ensure!(
            path.is_absolute() && path.is_file(),
...
fn runtime_state(te: bool, tp: Option<&str>, se: bool, sp: Option<&str>) -> String {
    ...
    } else if (!te || executable_ready(tp)) && (!se || executable_ready(sp)) {
        "ready".to_owned()
    } else {
        "degraded".to_owned()
    }
...
fn runtime_health(te: bool, tp: Option<&str>, se: bool, sp: Option<&str>) -> Option<String> {
    ...
    Some(if issues.is_empty() {
        if te || se {
            "configured local voice runtimes are available".to_owned()
        } else {
            "no local voice runtime is configured".to_owned()
        }
    } else {
        issues.join("; ")
    })
...
fn installation_id() -> Result<String> {
    let entry = Entry::new(INSTALLATION_SERVICE, INSTALLATION_ACCOUNT)?;
    match entry.get_password() {
        Ok(value) if Uuid::parse_str(value.trim()).is_ok() => Ok(value.trim().to_owned()),
        _ => {
...
fn load_lease_for_job(state: &Arc<AppState>, id: &str) -> Result<String> {
    ...
    let key: String = connection.query_row(
        "SELECT lease_credential_key FROM pod_provider_voice_jobs WHERE local_job_id=?1",
        params![id],
        |row| row.get(0),
    )?;
    load_lease(&key)
...
mod tests {
    ...
    fn provider_url_requires_https() {
        ...
        assert!(normalize_url("http://pod.example").is_err());
    }
}

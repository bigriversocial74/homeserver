...
#[tauri::command]
pub(crate) async fn homeserver_import_operational_data(request: Value) -> Result<Value, String> {
    post_json("/v1/operational-data/import", &request).await
}

#[tauri::command]
pub(crate) async fn homeserver_query_operational_data(request: Value) -> Result<Value, String> {
